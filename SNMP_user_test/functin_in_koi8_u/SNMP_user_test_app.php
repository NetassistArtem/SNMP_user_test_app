...
)
{
    ...
    $url = $host . "/bl/account_test?bl=1&switch={$switch_id}&port={$port_id}&user_id={$user_id}&cabletest={$cable_test}&link={$link}&warning={$warning}&notice={$notice}&information={$information}{$cable_l}&switch_data={$switch_data}";
    //   echo $url;
    $url_array = array(
        'full_url'=> $url,
        'host'=> $host,
        'address'=> "/bl/account_test",
        'params' => "bl=1&switch={$switch_id}&port={$port_id}&user_id={$user_id}&cabletest={$cable_test}&link={$link}&warning={$warning}&notice={$notice}&information={$information}{$cable_l}&switch_data={$switch_data}"
    );
    return $url_array;
...
)
{
    ...
    $notice = $style_class_array['notice'];
    $information = $style_class_array['information'];
    $url = $host . "/bl/account_test/history?bl=1&switch=$switch_id&port=$port_id&user_id={$user_id}&warning={$warning}&notice={$notice}&information={$information}";
    //   echo $url;
    $url_array = array(
        'full_url'=> $url,
        'host'=> $host,
        'address'=> "/bl/account_test/history",
        'params' => "bl=1&switch=$switch_id&port=$port_id&user_id={$user_id}&warning={$warning}&notice={$notice}&information={$information}"
    );
    return $url_array;
